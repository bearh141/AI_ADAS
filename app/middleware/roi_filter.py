...
# ── Kiểu module được hỗ trợ ──────────────────────────────────────────────────
ModuleType = Literal["lane", "sign", "object"]

# Giá trị mặc định nếu config.yaml không có mục roi
_DEFAULT_ROI = {
...
# Cache presets khi import (tránh đọc file mỗi lần predict)
_ROI_PRESETS: dict = _load_roi_presets()


def _get_roi_config(module: ModuleType, frame_h: int, frame_w: int) -> dict:
    """
    Trả về tọa độ pixel ROI (x1, y1, x2, y2) từ tỉ lệ đã cấu hình.

    Sơ đồ vùng mặc định:
    ┌──────────────────────────────┐  y=0
    │   [sign zone: 0–60% height] │
    │   [  center 20%–80% width ] │
    ├──────────────────────────────┤  y=40%
    │   [object zone: 30–90%]     │
    │   [  center 10%–90% width ] │
...
def crop_roi(frame: np.ndarray, module: ModuleType) -> tuple[np.ndarray, dict]:
    """
    Cắt frame theo vùng ROI của module.

    Returns:
        cropped  : ảnh đã cắt (dùng để predict)
        roi_meta : dict chứa offset (x1, y1) để map bbox về tọa độ gốc
    """
    h, w = frame.shape[:2]
...
def remap_detections(detections: list[dict], roi_meta: dict, module: ModuleType) -> list[dict]:
    """
    Dịch chuyển tọa độ bbox từ hệ tọa độ ROI về hệ tọa độ frame gốc.

    Hỗ trợ cả key "box" (lane/sign) lẫn "bbox" (object).
    """
    ...
    for det in detections:
        ...
        # --- lane / sign dùng key "box" ---
        if "box" in det:
            x1, y1, x2, y2 = det["box"]
            det["box"] = [x1 + ox, y1 + oy, x2 + ox, y2 + oy]

        # --- object dùng key "bbox" ---
        elif "bbox" in det:
...
def apply_roi(frame: np.ndarray, module: ModuleType) -> tuple[np.ndarray, dict]:
    """
    Shortcut: trả về (cropped_frame, roi_meta).
    Dùng trước khi gọi predict.
    """
...
def draw_roi_debug(frame: np.ndarray, module: ModuleType) -> np.ndarray:
    """
    Vẽ hình chữ nhật ROI lên frame để debug/visualize.
    """
